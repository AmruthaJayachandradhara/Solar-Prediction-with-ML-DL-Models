...
'''In the next session try log transofmring the target if it's skewed
Add a rule-based wrappermethod so it only predicts daylight hours and not nighttime hours since generation is zero during nighttime hours, this will make the model more robust and focused on learning the patterns during daylight hours when generation actually occurs
'''